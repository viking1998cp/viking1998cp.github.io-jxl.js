(function () {
  ...
  async function decode(img, isCSS, isSource) {
    ...
    const imageBlob = await res.blob(); // Lấy blob của ảnh từ URL
    workers[jxlSrc] = new Worker('jxl_dec.js');
    workers[jxlSrc].postMessage({ jxlSrc, imageBlob }); // Truyền blob vào worker để xử lý
  ...
  new MutationObserver(mutations => mutations.forEach(mutation => mutation.addedNodes.forEach(el => {
    if (el instanceof HTMLImageElement && el.dataset.jxlSrc && el.dataset.jxlSrc.endsWith('.jxl')) {
      el.onerror = () => decode(el, false, false);
    } else if (el instanceof HTMLSourceElement && el.dataset.jxlSrc && el.srcset.endsWith('.jxl')) {
      decode(el, false, true);
    } else if (el instanceof Element && getComputedStyle(el).backgroundImage && getComputedStyle(el).backgroundImage.endsWith('.jxl")')) {
